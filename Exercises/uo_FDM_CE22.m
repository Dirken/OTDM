...
isd = 1; icg = 2; irc = 0 ; nu = 0.1; % Search direction:
...
for k = 1:niter-1   dd = [dd, dk(:,k)'*dk(:,k+1)]; end % Direction orthogonality
rk=[rk,NaN]; gdk = [gdk,0]; dd = [dd, 0];
...
fprintf('   isd= %1d\n', isd);
fprintf('   k x(1) x(2) iW g''*d ||g|| r d''d\n');
for k = 1:niter
    fprintf('%5d %7.4f %7.4f %3d %+3.1e %4.2e %3.1e, %3.1e\n', k, xk(1,k), xk(2,k), iWk(k), gdk(k), norm(gk(:,k)), rk(k), dd(k));
end
